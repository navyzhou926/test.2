...
//TCP
    ...
    for (i = 0; i < 4; i++) 
    {
        sprintf(buffer, "./upload-folder/%s", *(filename + i));
        if ((dir = opendir(buffer)) != NULL) 
        {
            while ((ptr = readdir(dir)) != NULL)
            {
                ...
                {
                    sprintf(buffer, "./upload-folder/%s/%s",*(filename + i), ptr->d_name);
...
//TCP
    ...
    while ((recv(client_sock_tcp, buffer, BUFFER_SIZE, 0)) > 0)
    {
        ...
        {
            case '0':
                fd = received_file(buffer, len, "./upload-folder/PictureFile");
                break;
            case '1':
                fd = received_file(buffer, len, "./upload-folder/MediaFile");
                break;
            case '2':
                fd = received_file(buffer, len, "./upload-folder/TextFile");
                break;
            case '3':
                fd = received_file(buffer, len, "./upload-folder/OtherFile");
                #if 0
                if (buffer[1] == '1') 
                {
                    //if (fd != NULL) 

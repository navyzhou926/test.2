...
void ControlWin::slotready()
{
    ...
    qDebug() << "PORT >>>>>" <<port;
    QString ba(buf);
    if((strstr(buf,"startall") || strstr(buf,"start2")) && flag == 0)   //if receive "ON" from server -> execut Ad display function
    {
        OnON();
    }
    else if((strstr(buf,"stopall") || strstr(buf,"stop2")) && flag == 1) //if receive "OFF" from server -> close Ad display function
    {
        OnOFF();
    }
    else if(strstr(buf,"ready2") || strstr(buf,"readyall"))
    {
        disProcess->close();
        disProcess->start("./SyncBack/SyncBack");
